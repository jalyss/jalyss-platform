  ...
  return (
    <div className="container" style={{ border: 20}}>
      <div style={{ marginLeft: 600 }}>
        <AddButton content={'add new session'} onClick={() => navigate('newsession')} />
      </div>
      {Array.isArray(sessionStore.items) &&
        sessionStore.items.map((el, i) => (
          <Card key={i} style={{ marginBottom: 20, border: '1px solid purple', borderRadius: '10px', boxShadow: '0 0 5px rgba(0, 0, 0, 0.3)' }}>
            <Card.Body>
              <Card.Title style={{ fontSize: '20px', fontWeight: 'bold', marginBottom: '10px' }}>{el?.title}</Card.Title>
              <Card.Text style={{ fontSize: '16px' }}>{el.description}</Card.Text>
            </Card.Body>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: 10 }}>
              <AddButton size="small" content={'Delete'} onClick={() => handleDeletesessionsClick(el.id)} />
              <AddButton size="small" content={'Update'} onClick={() => navigate(`${el.id}`)} />
            </div>
          </Card>
        ))}
