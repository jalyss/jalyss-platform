...
import { fetchBookmark, removeBookmark } from "../../store/bookmarks";
...
const MyBookmarks = () => {
  ...
  useEffect(() => {
    if (authStore.me) {
      dispatch(fetchBookmark(authStore.me.id, skip, take));
...
console.log(bookmarksStore.bookmarks,"lol");
  ...
  const handleRemoveBookmark = (blogId) => {
    dispatch(removeBookmark(blogId))
      .then((res) => {
  ...
  return (
    <section style={{ backgroundColor: "#eee" }}>
      <div className="bookmarkListWrapper">
        {bookmarks.map((bookmark) => (
          <div className="blogItemWrapper" key={bookmark.blog.id} style={{ cursor: "pointer" }}>
          {bookmark.blog.cover ? (
            <img
          ...
          <div className="chip mt-3" onClick={() => navigate(`/blogs/${bookmark.blog.id}`)}>
            {bookmark.blog.category.nameEn}
          </div>
          <div className="d-flex flex-column gap-2" onClick={() => navigate(`/blogs/${bookmark.blog.id}`)}>
            <h5 style={{ margin: "20px", flex: "1" }}>{bookmark.blog.title}</h5>
          ...
          <div className="blogItemFooter d-flex justify-content-between">
            ...
            <Dropdown>
              ...
              <Dropdown.Menu size="sm" title="">
                <Dropdown.Item onClick={() => handleRemoveBookmark(bookmark.blog.id)}>
                  {t("bookmark.remove")}
                </Dropdown.Item>
              </Dropdown.Menu>
            </Dropdown>
