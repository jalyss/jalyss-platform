...
export class MvtsService {
  ...
  async create(dto: CreateMvtArticleDto) {
    ...
    if (dto.status === 'delivered') {
      ...
      let articlesBranchReceiver =
        await this.prisma.articlesByBranch.findUnique({
          where: {
            branchId_articleId: {
      ...
      await this.prisma.articlesByBranch.update({
  ...
  async update(id: string, dto: UpdateMvtArticleDto) {
    const mvt = await this.prisma.mvtArticle.update({
      where: { id },
      data: { ...dto, date: new Date(dto.date).toISOString() },
    });
    if (dto.status === 'delivered') {
      const articlesBranchSender =
        await this.prisma.articlesByBranch.findUnique({
          where: {
            branchId_articleId: {
              branchId: dto.branchSenderId,
